<?php
/*
* Plugin Name: Devvn Woocommerce Price List Filter
* Version: 1.0.0
* Description: Woocommerce Price List Filter gives you an advanced Price List Filter widget that you can add to your store’s sidebar. You can add unlimited price range options
* Author: Le Van Toan
* Author URI: http://levantoan.com
* Plugin URI: http://levantoan.com/san-pham/devvn-woocommerce-price-list-filter/
* Text Domain: devvn-pricelistfilter
...
if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
    ...
    class DevVN_Widget_Price_List_Filter extends WP_Widget {
        public function __construct() {
            ...
            parent::__construct( 'devvn_woocommerce_price_list_filter', __('DevVN - Woocommerce Price List Filter','devvn-pricelistfilter'), $widget_ops, $control_ops );
    ...
    add_action( 'admin_enqueue_scripts', 'devvn_pricelist_register_scripts', 1 );
    function devvn_pricelist_register_scripts() {
        $current_screen = get_current_screen();
        if(isset($current_screen->base) && $current_screen->base == 'widgets') {
            wp_enqueue_style('pricelist-css', plugins_url('assets/css/price-list-filter.css', __FILE__), array(), '1.0', 'all');
            wp_enqueue_script('pricelist-js', plugins_url('assets/js/price-list-filter.js', __FILE__), array('jquery', 'wp-util'), '1.0', true);

            $php_array = array(
                'URLhome' => home_url()
            );
            wp_localize_script('pricelist-js', 'devvn_price_array', $php_array);
        }
    }
}